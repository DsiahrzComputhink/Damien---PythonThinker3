...
FilePath = os.getcwd()
textfile = os.path.join(FilePath,"CT08 PythonThinker","ARCHIVE","Text Files","TOPIC-05 [ASCII]","[L12]-ASCII [hackmeifyoucan].txt")
...
def BruteDecryptFile(content: str):
    ...
    for i in range(95):
        decrypted = decryptSentence(FIRSTsentence,i + 1)
        print(f"{decrypted}","[",style.bcyan + f"Shifted by {i^i + 1}" + style.RESET,"]")
        time.sleep(num)
        if num > 0.25:
            num -= 0.01
